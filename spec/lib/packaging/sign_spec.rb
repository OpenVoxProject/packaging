...
DOC
    ...
    describe '#sign_all' do
      ...
      let(:rpms_not_to_sign) { ["#{rpm_directory}/aix/7.1/PC1/ppc/puppet-agent-5.5.3-1.aix7.1.ppc.rpm"] }
      let(:v3_rpms) { ["#{rpm_directory}/el/5/PC1/i386/puppet-agent-5.5.3-1.el5.i386.rpm", "#{rpm_directory}/sles/11/PC1/x86_64/puppet-agent-5.5.3-1.sles11.x86_64.rpm"] }
      let(:v4_rpms) { ["#{rpm_directory}/el/7/PC1/aarch64/puppet-agent-5.5.3-1.el7.aarch64.rpm"] }
      let(:rpms) { rpms_not_to_sign + v3_rpms + v4_rpms }
      let(:already_signed_rpms) { ["#{rpm_directory}/el/6/PC1/x86_64/puppet-agent-5.5.3-1.el6.x86_64.rpm"] }
      let(:noarch_rpms) {
        [
      ...
      it 'does not fail if there are no rpms to sign' do
        ...
        expect { Pkg::Sign::Rpm.sign_all(rpm_directory) }.not_to raise_error
      end
    end
  end
end
